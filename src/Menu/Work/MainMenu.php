...
class MainMenu
{
    ...
    public function build(): ItemInterface
    {
        ...
        $menu->addChild('Actions', ['route' => 'work.projects.actions'])
            ->setAttribute('class', 'nav-item')
            ->setLinkAttribute('class', 'nav-link');

        $menu->addChild('Tasks', ['route' => 'work.projects.tasks'])
            ->setExtra('routes', [
        ...
        $menu->addChild('Calendar', ['route' => 'work.projects.calendar'])
            ->setAttribute('class', 'nav-item')
            ->setLinkAttribute('class', 'nav-link');

        if ($this->auth->isGranted('ROLE_WORK_MANAGE_PROJECTS')) {
            $menu->addChild('Roles', ['route' => 'work.projects.roles'])
                ->setExtra('routes', [
